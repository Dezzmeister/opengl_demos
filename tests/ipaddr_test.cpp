...
void setup_ipaddr_tests() {
	describe("IPv4", []() {
		it("Parses an IP with four octets all set to zero", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(*result.ip == ipv4_addr(0, 0, 0, 0));
			expect_cond(result.out == L"0.0.0.0");
		});
		...
		it("Parses an IP with four octets all set to 255", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(*result.ip == ipv4_addr(255, 255, 255, 255));
			expect_cond(result.out == L"255.255.255.255");
		});
		...
		it("Parses an IP with four octets between 0 and 255", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(*result.ip == ipv4_addr(1, 23, 109, 45));
			expect_cond(result.out == L"1.23.109.45");
		});
		...
		it("Fails to parse an IP with three octets", []() {
			...
			expect_cond(! result.ip.has_value());
			expect_cond(result.out == L"1.23.45");
		});
		...
		it("Fails to parse an IP with two octets", []() {
			...
			expect_cond(! result.ip.has_value());
			expect_cond(result.out == L"23.45");
		});
		...
		it("Fails to parse an IP with one octet > 255", []() {
			...
			expect_cond(! result.ip.has_value());
			expect_cond(result.out == L"1.2.3.256");
		});
		...
		it("Fails to parse an IP with an invalid character", []() {
			...
			expect_cond(! result.ip.has_value());
			expect_cond(result.out == L"192.168.");
		});
		...
		it("Fails to parse an IP with a multi-digit octet with a leading zero", []() {
			...
			expect_cond(! result.ip.has_value());
			expect_cond(result.out == L"192.168.0");
		});
	});
	...
	describe("IPv6", []() {
		it("Parses an IP with no pieces elided and two h16s at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1234, 0x0191, 0xf, 0x1, 0x2929, 0xabcd, 0x8888, 0xdd));
			expect_cond(result.out == L"1234:0191:f:1:2929:abcd:8888:dd");
		});
		...
		it("Parses an IP with one piece elided and two h16s at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1234, 0x0191, 0, 0x1, 0x2929, 0xabcd, 0x8888, 0xdd));
			expect_cond(result.out == L"1234:0191::1:2929:abcd:8888:dd");
		});
		...
		it("Parses an IP with two pieces elided and two h16s at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1234, 0, 0, 0x1, 0x2929, 0xabcd, 0x8888, 0xdd));
			expect_cond(result.out == L"1234::1:2929:abcd:8888:dd");
		});
		...
		it("Parses an IP with three pieces elided and two h16s at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1234, 0, 0, 0, 0x2929, 0xabcd, 0x8888, 0xdd));
			expect_cond(result.out == L"1234::2929:abcd:8888:dd");
		});
		...
		it("Parses an IP with four pieces elided and two h16s at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1234, 0, 0, 0, 0, 0xabcd, 0x8888, 0xdd));
			expect_cond(result.out == L"1234::abcd:8888:dd");
		});
		...
		it("Parses an IP with five pieces elided and two h16s at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1234, 0, 0, 0, 0, 0, 0x8888, 0xdd));
			expect_cond(result.out == L"1234::8888:dd");
		});
		...
		it("Parses an IP with six pieces elided and one h16 at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1234, 0, 0, 0, 0, 0, 0, 0xdd));
			expect_cond(result.out == L"1234::dd");
		});
		...
		it("Parses an IP with one piece elided and one h16 at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1234, 0x5678, 0x9abc, 0xdef0, 0x1234, 0x5678, 0, 0xdd));
			expect_cond(result.out == L"1234:5678:9abc:def0:1234:5678::dd");
		});
		...
		it("Parses an IP with the first four pieces elided and two h16s at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0, 0, 0, 0, 0x1234, 0x5678, 0x9abc, 0xdef0));
			expect_cond(result.out == L"::1234:5678:9abc:def0");
		});
		...
		it("Parses an IP with the last four pieces elided", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1234, 0x5678, 0x9abc, 0xdef0, 0, 0, 0, 0));
			expect_cond(result.out == L"1234:5678:9abc:def0::");
		});
		...
		it("Parses an IP with all but the last piece elided and one h16 at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0, 0, 0, 0, 0, 0, 0, 0xf00));
			expect_cond(result.out == L"::f00");
		});
		...
		it("Parses an IP with all but the first piece elided", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0xf00, 0, 0, 0, 0, 0, 0, 0));
			expect_cond(result.out == L"f00::");
		});
		...
		it("Parses an IP with every piece elided", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr{});
			expect_cond(result.out == L"::");
		});
		...
		it("Parses an IP with no pieces elided and an IPv4 at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1234, 0x0191, 0xf, 0x1, 0x2929, 0xabcd, 0x8888, 0xdd));
			expect_cond(result.out == L"1234:0191:f:1:2929:abcd:136.136.0.221");
		});
		...
		it("Parses an IP with one piece elided and an IPv4 at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1234, 0x0191, 0, 0x1, 0x2929, 0xabcd, 0xc0a8, 0x1));
			expect_cond(result.out == L"1234:0191::1:2929:abcd:192.168.0.1");
		});
		...
		it("Parses an IP with two pieces elided and an IPv4 at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1234, 0, 0, 0x1, 0x2929, 0xabcd, 0, 0));
			expect_cond(result.out == L"1234::1:2929:abcd:0.0.0.0");
		});
		...
		it("Parses an IP with three pieces elided and an IPv4 at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1234, 0, 0, 0, 0x2929, 0xabcd, 0x0a00, 0x1));
			expect_cond(result.out == L"1234::2929:abcd:10.0.0.1");
		});
		...
		it("Parses an IP with four pieces elided and an IPv4 at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1234, 0, 0, 0, 0, 0xabcd, 0xc0a8, 0x1));
			expect_cond(result.out == L"1234::abcd:192.168.0.1");
		});
		...
		it("Parses an IP with all but the last piece elided and an IPv4 at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0, 0, 0, 0, 0, 0, 0xc0a8, 0x1));
			expect_cond(result.out == L"::192.168.0.1");
		});
		...
		it("Parses an IP with five pieces elided and an IPv4 at the end", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1234, 0, 0, 0, 0, 0, 0xc0a8, 0x1));
			expect_cond(result.out == L"1234::192.168.0.1");
		});
		...
		it("Parses an IP with two h16s at the end even when garbage follows", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1, 0x22, 0x333, 0x4444, 0x5, 0x6, 0x7, 0x8));
			expect_cond(result.out == L"1:22:333:4444:5:6:7:8");
		});
		...
		it("Parses an IP with an IPv4 at the end even when garbage follows", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1, 0x22, 0x333, 0x4444, 0x5, 0x6, 0xc0a8, 0x1));
			expect_cond(result.out == L"1:22:333:4444:5:6:192.168.0.1");
		});
		...
		it("Parses an IP with one piece elided and two h16s at the end even when garbage follows", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1, 0x22, 0x333, 0x4444, 0, 0x6, 0x7, 0x8));
			expect_cond(result.out == L"1:22:333:4444::6:7:8");
		});
		...
		it("Parses an IP with three pieces elided and an IPv4 at the end even when garbage follows", []() {
			...
			expect_cond(result.ip.has_value());
			expect_cond(result.ip == ipv6_addr(0x1, 0x22, 0, 0, 0, 0x6, 0xc0a8, 0x1));
			expect_cond(result.out == L"1:22::6:192.168.0.1");
		});
	});
}