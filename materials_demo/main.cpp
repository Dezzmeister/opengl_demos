...
static phong_material_properties materials[] = {
	// Emerald
	phong_material_properties(
...
phong_material_properties floor_mtl_props(
	glm::vec3(0.05, 0.05, 0.05),
	glm::vec3(0.5, 0.5, 0.5),
	glm::vec3(0.7, 0.7, 0.7),
	128 * 0.078125f
...
phong_material floor_mtl(floor_mtl_props);
...
struct world {
	...
	std::vector<std::unique_ptr<phong_material>> mats;
	point_light light_source;
	plane floor;

	world(std::vector<cube> _cubes, std::vector<std::unique_ptr<phong_material>> _mats, point_light _light_source, plane _floor) : 
...
static world create_world(event_buses &buses) {
	std::vector<cube> cubes;
	std::vector<std::unique_ptr<phong_material>> mats;
	const size_t num_materials = c_arr_size(materials);
	const float cube_size = 0.5f;
	const float spacing = 0.5f;
	const float width = (cube_size + spacing) * (num_materials - 1);

	for (size_t i = 0; i < num_materials; i++) {
		phong_material_properties &material_props = materials[i];
		std::unique_ptr<phong_material> mat = std::make_unique<phong_material>(material_props);
	...
	return world(std::move(cubes), std::move(mats), std::move(light_source), std::move(floor));
}
